    ...
    def shc(self):
        ...
        while True:
            ...
            if tuple(map(tuple, current_state)) in visited:
                return open_list, (len(open_list), heuristic(current_state, self.target)), step

            visited.add(tuple(map(tuple, current_state)))

            if current_state == self.target:
                open_list.append((current_state, heuristic(current_state, self.target), "Goal"))
                return open_list, (len(open_list), 0), step  
            ...
            for new_state, move in neighbors:
                h = heuristic(new_state, self.target)
                if h <= best_h:
            ...
            open_list.append((current_state, heuristic(current_state, self.target), best_move))
